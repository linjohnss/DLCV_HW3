...
class ConvNeXtFPN(nn.Module):
    ...
    def __init__(self, backbone, backbone_name="convnext_tiny"):
        ...
        if backbone_name == "convnext_tiny":
            self.in_channels_list = [96, 192, 384, 768]  # ConvNeXt-Tiny feature channels
        elif backbone_name == "convnext_small":
            self.in_channels_list = [96, 192, 384, 768]  # ConvNeXt-Small feature channels
        elif backbone_name == "convnext_base":
            self.in_channels_list = [128, 256, 512, 1024]  # ConvNeXt-Base feature channels
        elif backbone_name == "convnext_large":
            self.in_channels_list = [192, 384, 768, 1536]  # ConvNeXt-Large feature channels
        else:
...
class TransformerMaskDecoder(nn.Module):
    ...
    This module uses transformer layers to refine mask features before final prediction
    """
    def __init__(self, in_channels, dim_feedforward=2048, nhead=8, num_layers=2, dropout=0.1):
        super().__init__()
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(1, in_channels, 14, 14))
    ...
    def forward(self, x):
        ...
        x_2d = x_transformed.permute(0, 2, 1).view(batch_size, channels, height, width)
...
class EnhancedMaskRCNNPredictor(nn.Module):
    ...
    """
    def __init__(self, in_channels, dim_inner=256, num_classes=2, mask_head_type="default", use_transformer=False, transformer_layers=2, transformer_heads=8):
        super().__init__()
        ...
        if mask_head_type == "default":
            ...
            layers = [
                nn.Conv2d(in_channels, dim_inner, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
                nn.Conv2d(dim_inner, dim_inner, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
            ]
            ...
            layers = [
                nn.Conv2d(in_channels, wider_dim, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
                nn.Conv2d(wider_dim, wider_dim, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
                nn.Conv2d(wider_dim, dim_inner, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
            ]
        else:
        ...
        self.mask_pred = nn.Conv2d(dim_inner, num_classes, kernel_size=1, stride=1)
...
class CustomMaskRCNN(nn.Module):
    ...
    Wraps torchvision's Mask R-CNN implementation, providing an interface suitable for the current task
    """
    def __init__(self, num_classes=5, box_score_thresh=0.5, backbone="default", mask_head_type="default", use_transformer=False, transformer_layers=2, transformer_dim=256, transformer_heads=8, use_focal_loss=False, focal_loss_alpha=0.25, focal_loss_gamma=2.0):
        """
        ...
        Args:
            ...
            backbone (str): Backbone network type, supports 'default', 'resnet50', 'convnext_tiny',
                           'convnext_small', 'convnext_base', 'convnext_large'
            mask_head_type (str): Type of mask head to use ('default', 'deep', 'wider')
            use_transformer (bool): Whether to use transformer decoder in mask head
        ...
        if backbone == "default" or backbone == "resnet50":
            ...
            self.model = maskrcnn_resnet50_fpn_v2(
                ...
                box_detections_per_img=200,  # 增加檢測數量
                rpn_pre_nms_top_n_train=2000,  # 增加 RPN 訓練時的候選框數量
                rpn_post_nms_top_n_train=1000,  # 增加 RPN 訓練時的 NMS 後候選框數量
                rpn_pre_nms_top_n_test=1000,  # 增加 RPN 測試時的候選框數量
                rpn_post_nms_top_n_test=500,  # 增加 RPN 測試時的 NMS 後候選框數量
            )
            ...
            backbone_fpn = ConvNeXtFPN(convnext, backbone_name=backbone)
            
            # Get the same anchor_sizes and aspect_ratios as in default MaskRCNN
            ...
            rpn_head = torchvision.models.detection.rpn.RPNHead(
                backbone_fpn.out_channels, anchor_generator.num_anchors_per_location()[0]
            )
            ...
            box_head = torchvision.models.detection.faster_rcnn.TwoMLPHead(
                backbone_fpn.out_channels * box_roi_pool.output_size[0] ** 2, 1024
            )
            ...
            if mask_head_type == "default" and not use_transformer:
                ...
                mask_head = torchvision.models.detection.mask_rcnn.MaskRCNNHeads(
                    backbone_fpn.out_channels, [256, 256, 256, 256], 1
                )
                ...
                mask_layers = [256, 256, 256, 256]  # Default mask head configuration
                if mask_head_type == "deep":
                    mask_layers = [256, 256, 256, 256, 256, 256]  # Deeper mask head
                elif mask_head_type == "wider":
                    mask_layers = [512, 512, 256, 256]  # Wider mask head
                
                mask_head = torchvision.models.detection.mask_rcnn.MaskRCNNHeads(
                    backbone_fpn.out_channels, mask_layers, 1
                )
        ...
        self.model.roi_heads.score_thresh = 0.05  # 降低分數閾值
        self.model.roi_heads.nms_thresh = 0.5  # 調整 NMS 閾值

        # Replace the loss function with Focal Loss if requested
        if use_focal_loss:
            self._original_loss_fn = self.model.roi_heads.box_predictor.focal_loss if hasattr(self.model.roi_heads.box_predictor, 'focal_loss') else None
            self.model.roi_heads.box_predictor.focal_loss = FocalLoss(alpha=focal_loss_alpha, gamma=focal_loss_gamma)
            
            # Store the original loss function method for restoration if needed
            if not hasattr(self.model.roi_heads.box_predictor, '_original_forward'):
                self.model.roi_heads.box_predictor._original_forward = self.model.roi_heads.box_predictor.forward
                ...
                def forward_with_focal_loss(box_predictor, x):
                    ...
                    class_logits, box_regression = box_predictor._original_forward(x)
                    return class_logits, box_regression
                
                self.model.roi_heads.box_predictor.forward = forward_with_focal_loss.__get__(self.model.roi_heads.box_predictor)
    ...
    def log_model_parameters(self):
        ...
        if self.use_transformer:
            print(f"Using transformer with {self.transformer_layers} layers and {self.transformer_heads} heads")
        if self.use_focal_loss:
            print(f"Using Focal Loss with alpha={self.focal_loss_alpha}, gamma={self.focal_loss_gamma}")
        ...
        if self.backbone_type == "default" or self.backbone_type == "resnet50":
            ...
            for i, child in enumerate(self.model.backbone.body.children()):
                if i < layers:
                    ...
                    print(f"{'Freezing' if freeze else 'Unfreezing'} backbone layer {i}")
            ...
            if layers > 0:
                for param in backbone.features[0].parameters():
                    param.requires_grad = not freeze
                print(f"{'Freezing' if freeze else 'Unfreezing'} backbone embedding layer")
            ...
            for i in range(1, freeze_stages + 1):
                for param in backbone.features[i].parameters():
                    param.requires_grad = not freeze
                print(f"{'Freezing' if freeze else 'Unfreezing'} backbone stage {i}")
    ...
    def get_parameter_groups(self, backbone_lr_factor=0.1):
        ...
        Args:
            backbone_lr_factor (float): Learning rate factor for backbone network
